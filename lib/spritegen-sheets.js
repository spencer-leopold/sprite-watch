...
SpritegenSheets.prototype.generateSprite = function(spriteInfo, images) {
  ...
  var destImg, destImgDir = this.imgDest;
  var spriteSuffix = ~spriteInfo.name.indexOf('sprite') ? '.png' : '-sprite.png';

  if (!!this.streams) {
    destImgDir = this.imgDest;
    destImg = 'sprites/' + spriteInfo.name + spriteSuffix
  }
  else {
    destImgDir = this.imgDest + 'sprites/';
