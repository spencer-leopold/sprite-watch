...
SpritegenSheets.prototype.walkAndGenerate = function(src, spriteName) {
  ...
  return this.glob(src).then(function(files) {
    ...
    if (icons.length) {
      ...
      generateIcons = self.generateFontSprite(iconspriteInfo, icons);
    }
...
SpritegenSheets.prototype.generateFontSprite = function(spriteInfo, icons) {
  ...
  if (spriteInfo.name.indexOf('svg') === -1) {
    spriteInfo.name = 'svg-' + spriteInfo.name;
  }

  var streamObj = {};
  var end, start = new Date().getTime();
  var destFontDir = this.options.fontDest;
  var destFont = destFontDir + '/' + spriteInfo.name;
  var destCss = this.options.sheetDest + '/_' + spriteInfo.name + '.' + self.sheetFormat;
  ...
  return new Promise(function(resolve, reject) {
    var fontStream = svgicons2svgfont({
      ...
      callback: function(glyphs) {
        ...
        // Generate a listing of CSS variables
        var spritePath = url.relative(self.options.cssDest, destFont + '/' + spriteInfo.name);
        var spriteFileName = path.basename(destFont + '/' + spriteInfo.name + '.svg');
    ...
    fontStream.on('end', function(chunk) {
      ...
      if (!!self.streams) {
        ...
        fontStream.push(new File({
          path: spriteInfo.name + '/' + spriteInfo.name + '.svg',
          contents: new Buffer(data)
        }));

        fontStream.push(new File({
          path: spriteInfo.name + '/' + spriteInfo.name + '.ttf',
          contents: new Buffer(ttf.buffer)
        }));

        fontStream.push(new File({
          path: spriteInfo.name + '/' + spriteInfo.name + '.eot',
          contents: new Buffer(ttf2eot(new Uint8Array(ttf.buffer)).buffer)
        }));

        fontStream.push(new File({
          path: spriteInfo.name + '/' + spriteInfo.name + '.woff',
          contents: new Buffer(ttf2woff(new Uint8Array(ttf.buffer)).buffer)
        }));
        ...
        streamObj.imgFilename = spriteInfo.name,
      ...
      fs.writeFileSync(destFont + '/' + spriteInfo.name + '.ttf', new Buffer(ttf.buffer));

      fs.readFile(destFont +  '/' + spriteInfo.name + '.ttf', function(err, data) {
        fs.writeFileSync(destFont + '/' + spriteInfo.name + '.woff', new Buffer(ttf2woff(new Uint8Array(data)).buffer));
        fs.writeFileSync(destFont + '/' + spriteInfo.name + '.eot', new Buffer(ttf2eot(new Uint8Array(data)).buffer));
      });
    });

    // Setting the font destination 
    if (!self.streams) {
      fontStream.pipe(fs.createWriteStream(destFont + '/' + spriteInfo.name + '.svg'))
        .on('finish',function() {
