  ...
  if (typeof src === 'string') {
    ...
    if (!!this.streams) {
      return Promise.all(Object.keys(src)
        .map(function(sheet) {
          return self.walkAndGenerate(src[sheet]).then(function(info) {
            return info;
          }).catch(self.logger.log);
        })
      ).then(function(info) {
...
SpritegenSheets.prototype.watch = function() {
  ...
  if (!!this.streams) {
    ...
    Object.keys(this.src).map(function(sheet) {
      var dirs = [];
      var exts = [];
      var sheetSources = self.src[sheet];

      sheetSources.forEach(function(source) {
          ...
          if (!!self.streams) {
            for (var sheet in self.src) {
              ...
              sources.some(function(srcpath) {
                ...
                if (dirname === srcDir) {
                  self.walkAndGenerate(sources).catch(self.logger.log);
                  return true;
                }
...
SpritegenSheets.prototype.generateSprite = function(spriteInfo, images) {
  ...
  if (!!this.streams) {
    destImg = spriteInfo.name + '-sprite.png';
  }
  else {
    destImg = destImgDir + '/' + spriteInfo.name + '-sprite.png';
  }
