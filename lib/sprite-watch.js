...
function SpriteWatch(options, cb) {
  if (!cb) {
    done = false;
  }
  else {
  ...
  if (options && !options._) {
    ...
    if (!config.sprites) {
      console.error("No sprite configuration provided");
      process.exit(1);
    }

    this.options = assign({}, defaultOptions(), config.sprites.options);
    this.options.watch = options.watch;
    this.options.src = config.sprites.src;
    this.options.dest = config.sprites.dest;
  }
  ...
  this.sheetOutput = this.options.sheetOutput;
  this.sheetTemplate = this.options.sheetTemplate;
  this.sheetFormat = this.options.sheetFormat;

  async.each(this.src, function(dir) {
    this.walkAndGenerate(dir, options);
  }.bind(this));
...
SpriteWatch.prototype.watch = function() {
  ...
  var watcher = chokidar.watch(this.src, { cwd: process.cwd() });
...
SpriteWatch.prototype.generateSprite = function(spriteInfo, images) {
  ...
  if (this.dest) {
    var destCss = this.sheetOutput + '/_' + spriteInfo.name + '.' + this.sheetFormat;
  }
  else {
